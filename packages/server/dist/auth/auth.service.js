...
let AuthService = class AuthService {
    ...
    async exchangeCodeForToken(code, request) {
        ...
        try {
            ...
            throw new common_1.HttpException('인가 코드를 액세스 토큰으로 교환하는 중 오류가 발생했습니다.', common_1.HttpStatus.BAD_REQUEST);
        }
    }
    async getKakaoUserInfo(accessToken) {
        try {
            ...
            console.log('카카오 API 원본 응답:', JSON.stringify(response.data, null, 2));
            const kakaoId = response.data.id.toString();
            const userInfo = {
                userId: `kakao_${kakaoId}`,
                nickname: response.data.kakao_account?.profile?.nickname || '카카오 사용자',
                email: response.data.kakao_account?.email || null,
                profileImageUrl: response.data.kakao_account?.profile?.profile_image_url || null,
                provider: 'kakao',
                providerId: kakaoId,
            };
            ...
            throw new common_1.HttpException('카카오 사용자 정보를 조회하는 중 오류가 발생했습니다.', common_1.HttpStatus.BAD_REQUEST);
        }
    }
    async processUserLogin(userInfo) {
        try {
            ...
            throw new common_1.HttpException('사용자 정보를 처리하는 중 오류가 발생했습니다.', common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    async refreshTokens(refreshToken) {
        try {
            ...
            if (!user) {
                throw new common_1.HttpException('사용자를 찾을 수 없습니다.', common_1.HttpStatus.UNAUTHORIZED);
            }
            const newPayload = {
                sub: user.userId,
                userId: user.userId,
            ...
            throw new common_1.HttpException('토큰 갱신에 실패했습니다.', common_1.HttpStatus.UNAUTHORIZED);
        }
    }
